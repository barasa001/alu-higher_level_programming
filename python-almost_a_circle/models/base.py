...
class Base:
    """Defining a base with an ID"""

    __nb_objects = 0

    def __init__(self, id=None):
        """documentation for initialisation"""
        if id is not None:
            self.id = id
        else:
            Base.__nb_objects += 1
    ...
    def save_to_file(cls, list_objs):
        """function that writes string representation"""
        d = []
        with open(cls.__name__ + ".json", "w", encoding="utf-8") as f:
            if list_objs:
                for obj in list_objs:
                    d.append(obj.to_dictionary())
            f.write(cls.to_json_string(d))f.write(cls.to_json_string(d))
    ...
    def create(cls, **dictionary):
        """Return instance with attribbutes"""
